  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      try {
        ...
        ws.onopen = () => {
          console.log('TeleshopManagerAppointments WebSocket connected')
        }
        ...
        ws.onerror = (error) => {
          console.error('TeleshopManagerAppointments WebSocket error:', error)
        }

        ws.onclose = (event) => {
          console.log('TeleshopManagerAppointments WebSocket disconnected:', event.reason)
          if (!event.wasClean && isComponentMounted) {
            reconnectTimer = window.setTimeout(connectWebSocket, 5000)
          }
        }
      } catch (error) {
        console.error('Failed to create TeleshopManagerAppointments WebSocket:', error)
      }
    }
