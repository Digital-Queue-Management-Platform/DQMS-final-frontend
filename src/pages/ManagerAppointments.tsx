...
import {
  ...
  RefreshCw, Search, XCircle, MapPin, Phone, Globe
} from "lucide-react"
...
export default function ManagerAppointments() {
  ...
  async function fetchOutlets() {
    try {
      ...
      const mgr = raw ? JSON.parse(raw) : null
  ...
  async function loadData(outletList?: Outlet[]) {
    ...
    try {
      const ids = selectedOutlet === "all" ? list.map(o => o.id) : [selectedOutlet]
      const results: Row[] = []
      for (const oid of ids) {
        if (!oid) continue
        const res = await api.get(`/appointment/outlet/${oid}`)
        const outlet = list.find(o => o.id === oid)
        ;(res.data || []).forEach((a: Appointment) =>
          results.push({ ...a, outletName: outlet?.name, outletLocation: outlet?.location }))
      }
  ...
  useEffect(() => { if (outlets.length) loadData(outlets) }, [outlets, selectedOutlet, startDate, endDate])

  /* ── WebSocket real-time ── */
  useEffect(() => {
    let ws: WebSocket | null = null, timer: number | null = null, alive = true
    const connect = () => {
      if (!alive) return
      try {
        ...
        ws.onmessage = e => {
          try {
            ...
            if (["NEW_APPOINTMENT","APPOINTMENT_QUEUED","APPOINTMENT_CANCELLED"].includes(d.type)) loadData()
          } catch {}
        }
        ws.onclose = ev => { if (!ev.wasClean && alive) timer = window.setTimeout(connect, 5000) }
        ws.onerror = () => ws?.close()
      } catch {}
    }
    const interval = setInterval(() => loadData(), 30_000)
    connect()
    return () => { alive = false; clearInterval(interval); if (timer) clearTimeout(timer); if (ws?.readyState === WebSocket.OPEN) ws.close() }
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) { const qq = q.trim().toLowerCase(); d = d.filter(r => r.name?.toLowerCase().includes(qq) || r.mobileNumber?.includes(qq)) }
  ...
  useEffect(() => setPage(1), [startDate, endDate, selectedOutlet, selectedService, statusFilter, languageFilter, q])
  ...
  const badge   = (status: string) => `text-xs px-2.5 py-1 rounded-full font-semibold ${STATUS_STYLES[status] ?? "bg-gray-100 text-gray-600"}`
  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 flex items-center gap-2">
            <CalendarDays className="w-6 h-6 text-indigo-600" /> Appointments
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
            Your region's booked appointments · Last synced: {lastUpdated.toLocaleTimeString()}
          </p>
        </div>
        <button onClick={() => loadData()} disabled={loading}
          className="self-start sm:self-auto inline-flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 disabled:opacity-60 transition-colors">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label:"Total",     value:stats.total,     Icon:CalendarDays, cls:"text-indigo-600 bg-indigo-50" },
          ...
          <div key={label} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-3 sm:p-4 flex items-center gap-3">
            ...
            <div><p className="text-xl sm:text-2xl font-bold text-slate-900">{value}</p>
              <p className="text-[10px] sm:text-xs text-slate-500 font-medium">{label}</p></div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-3 sm:p-5 space-y-3">
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">

          {/* Outlet */}
          <div className="xl:col-span-1">
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Outlet</label>
            <AnimatedDropdown value={selectedOutlet} onChange={setSelectedOutlet}
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
                  className="w-full pl-8 pr-2 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
              </div>
          ...
          <div>
            ...
            <AnimatedDropdown value={statusFilter} onChange={setStatusFilter}
              options={[
                { value:"all", label:"Active (Booked+Queued)" }, { value:"booked", label:"Booked" },
                { value:"queued", label:"Queued" }, { value:"completed", label:"Completed" },
        ...
        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center">
            <div className="flex items-center justify-center gap-2 text-gray-500">
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-indigo-600" /> Loading…
            </div>
          </div>
        ) : pageData.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center">
          ...
          <div key={r.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-semibold text-indigo-700">{fmtDate(r.appointmentAt)} {fmtTime(r.appointmentAt)}</span>
            </div>
      ...
      <div className="hidden lg:block">
        ...
        <div ref={headerRef} className="bg-black/[0.06] rounded-xl mb-2 overflow-x-hidden px-4 py-1">
          <div className="grid grid-cols-8 text-left text-xs text-gray-500 uppercase font-semibold min-w-[960px] tracking-wide">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
          <div ref={bodyRef} className="overflow-x-auto" onScroll={onBodyScroll}>
            <div className="min-w-[960px] divide-y divide-slate-50">
              {loading ? (
                ...
                <div key={r.id} className="grid grid-cols-8 text-sm hover:bg-indigo-50/40 transition-colors min-w-[960px] rounded-lg">
                  <div className="px-3 py-3 col-span-2 whitespace-nowrap font-medium text-slate-700">
                    {fmtDate(r.appointmentAt)} <span className="text-indigo-600">{fmtTime(r.appointmentAt)}</span>
                  </div>
                  <div className="px-3 py-3 font-semibold text-slate-900">{r.name}</div>
                  <div className="px-3 py-3 text-slate-600">{r.mobileNumber}</div>
                  <div className="px-3 py-3 text-slate-600 truncate max-w-[130px]">{r.outletName || r.outletId}</div>
