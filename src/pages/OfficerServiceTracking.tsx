...
export default function OfficerServiceTracking() {
  ...
  const markComplete = async (caseRef: string) => {
    ...
    setCompleting(c => ({ ...c, [caseRef]: true }))
    try {
      const token = localStorage.getItem('officerToken')
      await api.post('/officer/service-case/complete', { refNumber: caseRef }, token ? { headers: { Authorization: `Bearer ${token}` } } : undefined)
      await load()
    } catch (e: any) {
      alert(e?.response?.data?.error || 'Failed to complete case')
    } finally { setCompleting(c => ({ ...c, [caseRef]: false })) }
  }
    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-4">
      ...
      {cases.length > 0 && !loading && (
        <div className="space-y-8">
          {cases.length > 1 && (
            <div className="bg-indigo-50 text-indigo-800 p-3 rounded-lg text-sm font-medium border border-indigo-200">
          ...
          {cases.map((data, index) => (
            <div key={data.refNumber} className="space-y-4 relative">
              {cases.length > 1 && (
                <div className="absolute -left-3 -top-3 w-8 h-8 bg-indigo-600 text-white rounded-full flex items-center justify-center font-bold shadow-md z-10 border-2 border-white">
                  {index + 1}
                </div>
              )}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
                  ...
                  <div className="flex gap-2 mt-3">
                    <button
                      ...
                      disabled={!(note[data.refNumber] || '').trim() || saving[data.refNumber]}
                    ...
                    >
                      ...
                      {saving[data.refNumber] ? 'Sending...' : 'Send Update'}
                    </button>
                    <button
                      onClick={() => markComplete(data.refNumber)}
                      disabled={data.status === 'completed' || completing[data.refNumber]}
                    ...
                    >
                      ...
                      {completing[data.refNumber] ? 'Completing...' : 'Mark Completed'}
                    </button>
                  </div>
                </div>
              )}
