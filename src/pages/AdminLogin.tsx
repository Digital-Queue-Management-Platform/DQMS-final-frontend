...
// Save credentials to the browser using the Credential Management API
const saveCredentials = async (id: string, pwd: string) => {
  if (!(window as any).PasswordCredential) return
  try {
    const cred = new (window as any).PasswordCredential({ id, password: pwd, name: id })
    await navigator.credentials.store(cred)
  } catch (_) {
    // Credential Management API not supported or user dismissed
  }
}
...
const AdminLogin = () => {
  ...
  useEffect(() => {
    ...
    // Auto-fill saved credentials from the browser's password manager
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      if (step === 1) {
        ...
        // Phase 2: Verify OTP and get token — save credentials here after confirmed success
        ...
        if (response.data.token) {
          ...
          // Prompt the browser to save the credentials
              ...
              <>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      id="mobile" name="mobile" type="tel" required maxLength={10}
              ...
              <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-center text-xl font-bold tracking-[0.35em]"
                      placeholder="• • • •"
                    />
                  </div>
