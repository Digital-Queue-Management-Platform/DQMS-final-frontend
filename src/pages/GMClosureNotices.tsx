...
export default function GMClosureNotices() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [noticesRes, outletsRes] = await Promise.all([
                ...
                api.get("/queue/outlets") // GM has island-wide access
            ])
            setNotices(noticesRes.data.notices || [])

            // Map the island-wide outlets for the dropdown
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            await api.post("/gm/closure-notices", {
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto min-h-screen">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><BellOff className="w-6 h-6 text-violet-600" />Closure Notices</h1>
                    <p className="text-sm text-gray-500 mt-1">Manage closure notices for all outlets in your regions</p>
                </div>
            ...
            {showForm && (
                <form onSubmit={handleCreate} className="mb-6 bg-violet-50 border border-violet-200 rounded-2xl p-5 space-y-4">
                    <h2 className="text-base font-semibold text-violet-900">New Closure Notice</h2>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required
                            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-500 focus:outline-none"
                            placeholder="e.g. Branch Closed – Strike" />
                    </div>
                    <div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {notice.outlet && (
                                        <p className="text-xs text-gray-500 flex items-center gap-1 mb-2">
                                            ...
                                            {notice.outlet.region && <span> • {notice.outlet.region.name}</span>}
                                        </p>
                                    )}
