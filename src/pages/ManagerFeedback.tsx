...
export default function ManagerFeedback() {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      try {
        ...
        ws.onopen = () => {
          console.log('ManagerFeedback WebSocket connected')
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === "RTOM_FEEDBACK_ALERT" || data.type === "NEW_TOKEN" || data.type === "TOKEN_COMPLETED") {
              fetchFeedback()
              fetchAlerts() // Refresh alerts when new 2-star feedback arrives

              // Show immediate notification for 2-star feedback alerts
        ...
        ws.onerror = (error) => {
          console.error('ManagerFeedback WebSocket error:', error)
        }

        ws.onclose = (event) => {
          console.log('ManagerFeedback WebSocket disconnected:', event.reason)
        ...
        console.error('Failed to create ManagerFeedback WebSocket:', error)
        if (isComponentMounted && connectionAttempts < maxReconnectAttempts) {
  ...
  const fetchFeedback = async () => {
    try {
      ...
      const response = await api.get(`/manager/feedback?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      console.log("RTOM feedbacks: ", response.data)
  ...
  const handleResolveFeedback = async (feedbackId: string, resolutionComment: string) => {
    try {
      ...
      await api.patch(`/manager/feedback/${feedbackId}/resolve`, {
        resolutionComment
      }, {
        headers: { Authorization: `Bearer ${token}` }
      })
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-bold text-gray-900">Customer Feedback (2-Star)</h1>
        </div>

        {/* 2-Star Feedback Alerts Notification Bell */}
        <div className="flex items-center gap-4">
          <button
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white rounded-2xl shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-gray-400">
            2-star customer feedback from your region will appear here for review and resolution
          </p>
        </div>
      )}
